...
describe('memoize plural type language', function () {
  ...
  before(function () {
    Polyglot = fakeDefaultExport('../', {
      'for-each': function (array, iterator, thisArg) {
        forEachCalls.push(arguments);
        forEach(array, iterator, thisArg);
      }
    });
  });
  after(function () {
  ...
  it('should memoize the plural type language once and call forEach expected amount of times', function () {
    ...
    expect(forEachCalls.length).to.equal(3);
    expect(forEachCalls[0][0]).to.eql({ test_phrase: '%{smart_count} Name |||| %{smart_count} Names' });
    expect(forEachCalls[1][0]).to.eql({ germanLike: ['x1'] });
    expect(forEachCalls[2][0]).to.eql(['x1']);
  });
});
