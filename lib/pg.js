...
!function(root) {
  ...
  // ### poloyglot = new Polyglot()
  //
  // Construct an instance of polyglot
  ...
  Polyglot.prototype.extend = function(morePhrases) {
    ...
    for (var key in morePhrases) {
      if (morePhrases.hasOwnProperty(key)) {
        self.phrases[key] = morePhrases[key];
      }
    }
  };
  ...
  Polyglot.prototype.t = function(key, options) {
    ...
    var result;
    options = options || {};
    var phrase = self.phrases[key] || options._ || '';
    if (phrase === '') {
      ...
      result = choosePluralForm(phrase, self.currentLocale, options.smart_count);
  ...
  Polyglot.prototype.pluralize = function(noun, count) {
    ...
    return self.t(key, {smart_count: count});
  };
  ...
  //     console.log(Handlebars.helpers.t);
  //     // => function(){...}
  Polyglot.prototype.registerHandlebars = function(handlebars) {
    var self = this;
    ...
    var fn = function(key, block) { 
      return self.t(key, block.hash); 
    };
    handlebars.registerHelper('t', fn);

    // ### pluralize
    //
    //     <p>{{pluralize "car" smart_count=carCollection}}</p>
    //
    // gives:
    //
    //     <p>3 cars</p>
    fn = function(noun, block) {
      return self.pluralize(noun, block.hash.smart_count);
    };
    handlebars.registerHelper('pluralize', fn);
